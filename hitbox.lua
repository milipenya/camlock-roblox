...
local Settings = {
	...
	TabVisibility = { Aim = true, Debug = true, Misc = true, Settings = true },
}
...
local DEFAULT_SETTINGS = {
	...
	UIScale = 1,
	Theme = "Neon",
	Keybinds = { TogglePanel = Enum.KeyCode.RightShift, PlayerAimbot = nil, NpcAimbot = nil },
	TabVisibility = { Aim = true, Debug = true, Misc = true, Settings = true },
}
...
local SavedSettingsSnapshot = nil
local PendingKeybind = nil
local ProfileName = "Combat"
local Profiles = { Combat = {}, Debug = {}, Testing = {} }
local ThemePalettes = {
	Neon = { Background=Color3.fromRGB(7,8,12), Panel=Color3.fromRGB(11,12,18), Panel2=Color3.fromRGB(16,17,25), Card=Color3.fromRGB(18,20,29), CardHover=Color3.fromRGB(27,22,34), Stroke=Color3.fromRGB(45,48,62), Pink=Color3.fromRGB(255,35,115), PinkDark=Color3.fromRGB(170,20,75), White=Color3.fromRGB(245,245,250), Gray=Color3.fromRGB(150,153,165), Gray2=Color3.fromRGB(95,99,112), Green=Color3.fromRGB(70,220,140), Red=Color3.fromRGB(255,70,90), Yellow=Color3.fromRGB(255,210,80) },
	Cyber = { Background=Color3.fromRGB(5,10,14), Panel=Color3.fromRGB(8,17,23), Panel2=Color3.fromRGB(12,25,32), Card=Color3.fromRGB(15,31,38), CardHover=Color3.fromRGB(18,48,55), Stroke=Color3.fromRGB(36,76,82), Pink=Color3.fromRGB(35,235,220), PinkDark=Color3.fromRGB(15,145,140), White=Color3.fromRGB(235,250,250), Gray=Color3.fromRGB(145,170,175), Gray2=Color3.fromRGB(78,105,110), Green=Color3.fromRGB(70,230,155), Red=Color3.fromRGB(255,80,95), Yellow=Color3.fromRGB(255,220,90) },
	Mono = { Background=Color3.fromRGB(12,12,12), Panel=Color3.fromRGB(18,18,18), Panel2=Color3.fromRGB(25,25,25), Card=Color3.fromRGB(31,31,31), CardHover=Color3.fromRGB(43,43,43), Stroke=Color3.fromRGB(65,65,65), Pink=Color3.fromRGB(235,235,235), PinkDark=Color3.fromRGB(160,160,160), White=Color3.fromRGB(245,245,245), Gray=Color3.fromRGB(165,165,165), Gray2=Color3.fromRGB(95,95,95), Green=Color3.fromRGB(120,220,150), Red=Color3.fromRGB(240,100,100), Yellow=Color3.fromRGB(240,210,100) },
}
...
do
	...
	local AimCategoryButton = createCategoryButton(
		HomePage,
	...
	local DebugCategoryButton = createCategoryButton(
		HomePage,
	...
	local MiscCategoryButton = createCategoryButton(
		HomePage,
	...
	local SettingsCategoryButton = createCategoryButton(
		HomePage,
	...
	local ServerInfoCategoryButton = createCategoryButton(
		...
		"SERVER INFO",
		"JobId, PlaceId, players and NPC count",
		"▣",
		function() showPage("ServerInfo") end
	)
end
...
--==================================================
-- EXTENDED SETTINGS / SERVER INFO HELPERS
--==================================================

local openPanel, closePanel
local MainUIScale = Instance.new("UIScale")
MainUIScale.Scale = Settings.UIScale
MainUIScale.Parent = Main

local function cloneTable(source)
	local result = {}
	for key, value in pairs(source) do result[key] = type(value) == "table" and cloneTable(value) or value end
	return result
end

local function saveSettingsSnapshot()
	SavedSettingsSnapshot = cloneTable(Settings)
end

local function applyUITransparency()
	local amount = math.clamp(Settings.UITransparency or 0, 0, 0.8)
	for _, object in ipairs(Main:GetDescendants()) do
		if object:IsA("GuiObject") then
			local base = object:GetAttribute("BaseBackgroundTransparency")
			if base == nil then base = object.BackgroundTransparency object:SetAttribute("BaseBackgroundTransparency", base) end
			if type(base) == "number" and base < 1 then object.BackgroundTransparency = math.clamp(base + amount, 0, 1) end
		end
	end
end

local function applyTheme(themeName)
	local palette = ThemePalettes[themeName] or ThemePalettes.Neon
	local old = COLORS
	for _, object in ipairs(ScreenGui:GetDescendants()) do
		if object:IsA("GuiObject") then
			local bg = object.BackgroundColor3
			if bg == old.Background then object.BackgroundColor3 = palette.Background elseif bg == old.Panel then object.BackgroundColor3 = palette.Panel elseif bg == old.Panel2 then object.BackgroundColor3 = palette.Panel2 elseif bg == old.Card then object.BackgroundColor3 = palette.Card elseif bg == old.CardHover then object.BackgroundColor3 = palette.CardHover end
			if object:IsA("TextLabel") or object:IsA("TextButton") then
				if object.TextColor3 == old.White then object.TextColor3 = palette.White elseif object.TextColor3 == old.Gray then object.TextColor3 = palette.Gray elseif object.TextColor3 == old.Gray2 then object.TextColor3 = palette.Gray2 elseif object.TextColor3 == old.Pink then object.TextColor3 = palette.Pink end
			end
		elseif object:IsA("UIStroke") then
			if object.Color == old.Pink then object.Color = palette.Pink elseif object.Color == old.PinkDark then object.Color = palette.PinkDark elseif object.Color == old.Stroke then object.Color = palette.Stroke end
		end
	end
	for key, value in pairs(palette) do COLORS[key] = value end
	Settings.Theme = themeName
	MainStroke.Color = COLORS.Pink OpenStroke.Color = COLORS.Pink AimFOVStroke.Color = COLORS.Pink
	applyUITransparency()
end

local function setTabVisibility(tabName, visible)
	Settings.TabVisibility[tabName] = visible
	local buttons = {Aim=AimCategoryButton, Debug=DebugCategoryButton, Misc=MiscCategoryButton, Settings=SettingsCategoryButton}
	if buttons[tabName] then buttons[tabName].Visible = visible end
	if Pages[tabName] and not visible and CurrentPage == Pages[tabName] then showPage("Home") end
end

local function refreshTabVisibility()
	for tabName, visible in pairs(Settings.TabVisibility) do setTabVisibility(tabName, visible) end
end
...
local function runKeybindAction(action)
	if action == "TogglePanel" then if IsOpen then closePanel() else openPanel() end
	elseif action == "PlayerAimbot" then Settings.PlayerAimbot = not Settings.PlayerAimbot if not Settings.PlayerAimbot then CurrentTarget = nil end
	elseif action == "NpcAimbot" then Settings.NpcAimbot = not Settings.NpcAimbot if not Settings.NpcAimbot then CurrentNpcTarget = nil end end
end

local function createSettingsAction(parent, y, title, description, callback)
	local button = Instance.new("TextButton")
	button.Size = UDim2.new(1,-16,0,47) button.Position = UDim2.fromOffset(8,y) button.BackgroundColor3 = COLORS.Card button.BorderSizePixel = 0 button.Text = "" button.ZIndex = 12 button.Parent = parent
	local corner = Instance.new("UICorner") corner.CornerRadius = UDim.new(0,9) corner.Parent = button
	local titleLabel = Instance.new("TextLabel") titleLabel.Name = "ActionTitle" titleLabel.BackgroundTransparency = 1 titleLabel.Position = UDim2.fromOffset(12,6) titleLabel.Size = UDim2.new(1,-24,0,17) titleLabel.Text = title titleLabel.TextColor3 = COLORS.White titleLabel.TextSize = 9 titleLabel.Font = Enum.Font.GothamBold titleLabel.TextXAlignment = Enum.TextXAlignment.Left titleLabel.ZIndex = 13 titleLabel.Parent = button
	local descLabel = Instance.new("TextLabel") descLabel.BackgroundTransparency = 1 descLabel.Position = UDim2.fromOffset(12,24) descLabel.Size = UDim2.new(1,-24,0,14) descLabel.Text = description descLabel.TextColor3 = COLORS.Gray descLabel.TextSize = 7 descLabel.Font = Enum.Font.Gotham descLabel.TextXAlignment = Enum.TextXAlignment.Left descLabel.ZIndex = 13 descLabel.Parent = button
	styleButton(button) button.Activated:Connect(callback) return button
end

local function setActionTitle(button, text)
	local label = button and button:FindFirstChild("ActionTitle") if label then label.Text = text end
end
...
createPageHeader(ServerInfoPage, "SERVER INFO", "Current place and session information")
local ServerInfoText = Instance.new("TextLabel")
ServerInfoText.BackgroundColor3 = COLORS.Panel2 ServerInfoText.BorderSizePixel = 0 ServerInfoText.Position = UDim2.fromOffset(8,52) ServerInfoText.Size = UDim2.new(1,-16,0,190) ServerInfoText.TextColor3 = COLORS.White ServerInfoText.TextSize = 10 ServerInfoText.Font = Enum.Font.Code ServerInfoText.TextXAlignment = Enum.TextXAlignment.Left ServerInfoText.TextYAlignment = Enum.TextYAlignment.Top ServerInfoText.ZIndex = 12 ServerInfoText.Parent = ServerInfoPage
local sic = Instance.new("UICorner") sic.CornerRadius = UDim.new(0,10) sic.Parent = ServerInfoText
local sip = Instance.new("UIPadding") sip.PaddingLeft = UDim.new(0,12) sip.PaddingTop = UDim.new(0,12) sip.Parent = ServerInfoText

local function countNPCs()
	local count = 0
	for _, model in ipairs(workspace:GetDescendants()) do
		if model:IsA("Model") and model ~= LocalPlayer.Character and model:FindFirstChildOfClass("Humanoid") and not Players:GetPlayerFromCharacter(model) then count += 1 end
	end
	return count
end

local function updateServerInfo()
	local jobId = game.JobId if jobId == "" then jobId = "Studio / local session" end
	ServerInfoText.Text = "SERVER INFO\n\nJobId        : " .. jobId .. "\nPlaceId      : " .. tostring(game.PlaceId) .. "\nPlayer Count : " .. tostring(#Players:GetPlayers()) .. " / " .. tostring(Players.MaxPlayers) .. "\nNPC Count    : " .. tostring(countNPCs())
end

createSettingsAction(ServerInfoPage,253,"REFRESH SERVER INFO","Update JobId, PlaceId, players and NPC count",updateServerInfo)
updateServerInfo()
...
createPageHeader(SettingsPage, "SETTINGS", "Panel, keybind and configuration management")
local SettingsStatus = Instance.new("TextLabel")
SettingsStatus.BackgroundTransparency = 1 SettingsStatus.Position = UDim2.fromOffset(10,47) SettingsStatus.Size = UDim2.new(1,-20,0,20) SettingsStatus.TextColor3 = COLORS.Gray SettingsStatus.TextSize = 8 SettingsStatus.Font = Enum.Font.Gotham SettingsStatus.ZIndex = 12 SettingsStatus.Parent = SettingsPage
local function refreshSettingsStatus() SettingsStatus.Text = "Profile: " .. ProfileName .. "   •   Theme: " .. Settings.Theme .. "   •   Scale: " .. string.format("%.1fx",Settings.UIScale) end
createToggle(SettingsPage,70,"UI ANIMATIONS","Enable page and button animations",function() return Settings.UIAnimations end,function(v) Settings.UIAnimations=v end)
local CompactToggle = createToggle(SettingsPage,129,"COMPACT UI","Use a smaller panel on PC",function() return Settings.CompactUI end,function(v) Settings.CompactUI=v updatePanelSize() end)
local SaveSettingsButton = createSettingsAction(SettingsPage,188,"SAVE SETTINGS","Save the current configuration for this session",function() saveSettingsSnapshot() setActionTitle(SaveSettingsButton,"SAVE SETTINGS  •  SAVED") end)
local KeybindButton = createSettingsAction(SettingsPage,239,"KEYBIND MANAGER","Click, then press a key to bind panel toggle",function() PendingKeybind="TogglePanel" setActionTitle(KeybindButton,"KEYBIND MANAGER  •  PRESS A KEY") end)
local TransparencyButton = createSettingsAction(SettingsPage,290,"UI TRANSPARENCY","Cycle interface transparency from 0% to 70%",function() Settings.UITransparency += .1 if Settings.UITransparency > .7 then Settings.UITransparency=0 end applyUITransparency() setActionTitle(TransparencyButton,"UI TRANSPARENCY  •  "..math.floor(Settings.UITransparency*100).."%") end)
local ScaleButton = createSettingsAction(SettingsPage,341,"UI SCALE","Cycle interface scale from 0.8x to 1.3x",function() Settings.UIScale += .1 if Settings.UIScale > 1.3 then Settings.UIScale=.8 end Settings.UIScale=math.floor(Settings.UIScale*10+.5)/10 MainUIScale.Scale=Settings.UIScale setActionTitle(ScaleButton,"UI SCALE  •  "..string.format("%.1fx",Settings.UIScale)) refreshSettingsStatus() end)
local ThemeButton = createSettingsAction(SettingsPage,392,"THEME SELECTOR","Cycle Neon, Cyber and Mono themes",function() local names={"Neon","Cyber","Mono"} local i=table.find(names,Settings.Theme) or 1 i=i%#names+1 applyTheme(names[i]) setActionTitle(ThemeButton,"THEME SELECTOR  •  "..Settings.Theme) refreshSettingsStatus() end)
local AimTabToggle = createToggle(SettingsPage,443,"AIM TAB","Show the Aim tab on the home screen",function() return Settings.TabVisibility.Aim end,function(v) setTabVisibility("Aim",v) end)
local DebugTabToggle = createToggle(SettingsPage,502,"DEBUG TAB","Show the Debug tab on the home screen",function() return Settings.TabVisibility.Debug end,function(v) setTabVisibility("Debug",v) end)
local MiscTabToggle = createToggle(SettingsPage,561,"MISC TAB","Show the Misc tab on the home screen",function() return Settings.TabVisibility.Misc end,function(v) setTabVisibility("Misc",v) end)
local SettingsTabToggle = createToggle(SettingsPage,620,"SETTINGS TAB","Show the Settings tab on the home screen",function() return Settings.TabVisibility.Settings end,function(v) setTabVisibility("Settings",v) end)
local ProfileButton = createSettingsAction(SettingsPage,679,"CONFIG PROFILES","Cycle Combat, Debug and Testing profiles",function()
	Profiles[ProfileName] = cloneTable(Settings)
	local names={"Combat","Debug","Testing"}
	local i=table.find(names,ProfileName) or 1
	i=i%#names+1
	ProfileName=names[i]
	if next(Profiles[ProfileName])==nil then Profiles[ProfileName]=cloneTable(Settings) end
	for k,v in pairs(Profiles[ProfileName]) do Settings[k]=type(v)=="table" and cloneTable(v) or v end
	MainUIScale.Scale=Settings.UIScale
	applyTheme(Settings.Theme)
	applyUITransparency()
	refreshTabVisibility()
	refreshSettingsStatus()
	setActionTitle(ProfileButton,"CONFIG PROFILES  •  "..ProfileName)
end)
local ResetPositionButton = createSettingsAction(SettingsPage,730,"RESET UI POSITION","Center the panel on screen",function() Main.AnchorPoint=Vector2.new(.5,.5) Main.Position=UDim2.fromScale(.5,.5) end)
local ResetSettingsButton = createSettingsAction(SettingsPage,781,"RESET SETTINGS","Restore all default settings",function()
	for k,v in pairs(DEFAULT_SETTINGS) do Settings[k]=type(v)=="table" and cloneTable(v) or v end
	ProfileName="Combat" Profiles={Combat={},Debug={},Testing={}}
	MainUIScale.Scale=Settings.UIScale
	applyTheme(Settings.Theme)
	applyUITransparency()
	updatePanelSize()
	refreshAimPart() refreshRange() refreshFOV() refreshSmooth()
	if DebugOverlayToggle then DebugOverlayToggle.Refresh() end
	if TargetDebugToggle then TargetDebugToggle.Refresh() end
	if PerformanceDebugToggle then PerformanceDebugToggle.Refresh() end
	CompactToggle.Refresh()
	AimTabToggle.Refresh()
	DebugTabToggle.Refresh()
	MiscTabToggle.Refresh()
	SettingsTabToggle.Refresh()
	refreshTabVisibility()
	refreshSettingsStatus()
end)
local AimKeybindsButton = createSettingsAction(SettingsPage,832,"AIM KEYBINDS","Assign P to Player Aimbot and N to NPC Aimbot",function() Settings.Keybinds.PlayerAimbot=Enum.KeyCode.P Settings.Keybinds.NpcAimbot=Enum.KeyCode.N setActionTitle(AimKeybindsButton,"AIM KEYBINDS  •  P / N ASSIGNED") end)
local SettingsInfo = Instance.new("TextLabel")
SettingsInfo.BackgroundTransparency=1 SettingsInfo.Position=UDim2.fromOffset(10,700) SettingsInfo.Size=UDim2.new(1,-20,0,45) SettingsInfo.Text="Save Settings stores a session snapshot.\nPermanent cross-session saving requires a server-side DataStore." SettingsInfo.TextColor3=COLORS.Gray SettingsInfo.TextSize=8 SettingsInfo.Font=Enum.Font.Gotham SettingsInfo.TextXAlignment=Enum.TextXAlignment.Left SettingsInfo.TextYAlignment=Enum.TextYAlignment.Top SettingsInfo.Parent=SettingsPage
refreshSettingsStatus()
	...
	function(input, processed)
		...
		if PendingKeybind then
			if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode ~= Enum.KeyCode.Unknown then
				Settings.Keybinds[PendingKeybind] = input.KeyCode
				PendingKeybind = nil
				setActionTitle(KeybindButton, "KEYBIND MANAGER  •  " .. formatKeyCode(Settings.Keybinds.TogglePanel))
			end
			return
		end
		for action, keyCode in pairs(Settings.Keybinds) do
			if keyCode and input.KeyCode == keyCode then runKeybindAction(action) return end
		end
	end
)
...
local ServerInfoTimer = 0
table.insert(Connections, RunService.Heartbeat:Connect(function(dt) ServerInfoTimer += dt if ServerInfoTimer >= 1 then ServerInfoTimer=0 updateServerInfo() end end))
...
setActionTitle(KeybindButton,"KEYBIND MANAGER  •  "..formatKeyCode(Settings.Keybinds.TogglePanel))
setActionTitle(TransparencyButton,"UI TRANSPARENCY  •  "..math.floor(Settings.UITransparency*100).."%")
setActionTitle(ScaleButton,"UI SCALE  •  "..string.format("%.1fx",Settings.UIScale))
setActionTitle(ThemeButton,"THEME SELECTOR  •  "..Settings.Theme)
setActionTitle(AimKeybindsButton,"AIM KEYBINDS  •  P / N ASSIGN")
