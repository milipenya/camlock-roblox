...
local function createValueButton(name, value, callback)
	local Button = createButton(name .. ": x" .. string.format("%.1f", value))
...
MarkerButton.MouseButton1Click:Connect(function()
	...
	setToggle(MarkerButton, "ENEMY MARKERS", Settings.Markers)
end)
...
local SmoothButton = createValueButton(
	"AIM SMOOTH",
...
local ShowPlayerButton = createButton("SHOW PLAYER HITBOXES: OFF")

ShowPlayerButton.MouseButton1Click:Connect(function()
	Settings.ShowPlayerHitboxes = not Settings.ShowPlayerHitboxes
	setToggle(
		ShowPlayerButton,
...
local ShowNpcButton = createButton("SHOW NPC HITBOXES: OFF")

ShowNpcButton.MouseButton1Click:Connect(function()
	Settings.ShowNpcHitboxes = not Settings.ShowNpcHitboxes
	setToggle(
		ShowNpcButton,
		"SHOW NPC HITBOXES",
...
local function createEnemyESP(player)
	...
	if not highlight then
		...
		highlight.FillColor = Color3.fromRGB(255, 35, 65)
	...
	if root and Settings.Markers then
		local marker = root:FindFirstChild("AdminEnemyMarker")

		if not marker then
			marker = Instance.new("BillboardGui")
			marker.Name = "AdminEnemyMarker"
			marker.Size = UDim2.fromOffset(70, 70)
			...
			arrow.Text = "▼"
			arrow.TextColor3 = Color3.fromRGB(255, 40, 70)
...
local function clearEnemyVisuals()
	for _, player in ipairs(Players:GetPlayers()) do
	...
	for _, player in ipairs(Players:GetPlayers()) do
		if player ~= LocalPlayer and player.Character then
			if Settings.ShowPlayerHitboxes then
				...
				if root then
					local box = root:FindFirstChild("AdminPlayerHitbox")
	...
	for _, model in ipairs(workspace:GetDescendants()) do
		if model:IsA("Model")
			...
			if Settings.ShowNpcHitboxes then
				...
				if root then
					local box = root:FindFirstChild("AdminNpcHitbox")
...
RunService.RenderStepped:Connect(function()
	...
	if Settings.Aimbot then
		local target = getEnemyTarget()

		if target then
			local cameraPosition = Camera.CFrame.Position
			local direction = (target.Position - cameraPosition).Unit
...
OpenButton.Position = UDim2.new(1, -65, 0, 20)
OpenButton.BackgroundColor3 = Color3.fromRGB(15, 15, 18)
OpenButton.Text = "⚙"
OpenButton.TextColor3 = Color3.fromRGB(255, 55, 100)
